...
type ListenerCallback = (type: EventType, event: Event, options: EventOptions) => { remove: boolean } | void;
...
class ECEventManager {
   ...
   * @returns a function for removing the event listeners created with this specific call to the `add` method. Event listeners associated with the combination of properties passed to `removeOptions` will be removed. Passing `[]` to a property of `removeOptions` will remove all the listeners associated with that specific property in combination with the other properties. Ultimately, passing `[]` to all properties (or leaving `removeOptions` undefined) will remove every listener associated with this call.
   */
  add(
...
const { remove } = EventManager.add('click', window, () => {});
remove();
